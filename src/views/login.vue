...
<template>
  ...
  <n-card size="large" style="--padding-bottom: 30px">
    ...
    <n-button type="primary" size="large" block :loading="loading" :disabled="disabled" @click="handleReg">Register</n-button>
    <br>
  </n-card>
</template>
...
const handleLogin = async (e: Event): Promise<void> => {
  ...
  loading.value = true
  try {
    await useToken(model.value.username, model.value.password)
...
const handleReg = async (e: Event): Promise<void> => {
  ...
  loading.value = true
  try {
    await useReg(model.value.username, model.value.password)
    const route = router.currentRoute.value
    const redirect = route.query.redirect?.toString()
    await router.replace(redirect ?? route.redirectedFrom?.fullPath ?? '/')
  } catch (e) {
